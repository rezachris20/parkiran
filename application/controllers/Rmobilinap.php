 ...
 * 
 */
class Rmobilinap extends CI_Controller
{
	...
	function laporan_pdf()
	{
		...
		$this->load->view('admin/report/v_mobil_inap_pdf',$data);
		...
		$this->dompdf->render();
		$this->dompdf->stream("Laporan Mobil Inap ".$dari.'-'.$sampai, array('Attachment' => 0));
	}
	...
	function laporan_excel()
	{
    	...
    	$style_row = array(
   		...
   		$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA MOBIL INAP");// Set kolom A1 dengan tulisan "DATA MOBIL INAP"
   		$excel->setActiveSheetIndex(0)->setCellValue('A2', "$cdari s/d $csampai");  // Set kolom A2 dengan tulisan "dari tanggal yang dipilih sampai tanggal yang dipili"
    	$excel->getActiveSheet()->mergeCells('A1:K1'); // Set Merge Cell pada kolom A2 sampai K2
    	...
    	$no = 1;
    	$numrow = 5;
    	foreach($data as $l){ // Lakukan looping pada variabel $data as $l
            $total = $total+$l->bayar;
    	...
    	$excel->getActiveSheet(0)->setTitle("Report Mobil Inap");
    	$excel->setActiveSheetIndex(0);
    	// Proses file excel
    	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    	header("Content-Disposition: attachment; filename='Report Mobil Inap $cdari s/d $csampai.xlsx' "); // Set nama file excel nya
